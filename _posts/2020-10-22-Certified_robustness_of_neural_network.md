...
令$\{x,y\}_{i=1,\cdots,n}$表示原始输入数据集，$x \in R^d, y \in \{1,\cdots k\}$。神经网络$f$的可验证鲁棒性评估流程如图1.1所示。
...
$$f_i^L(x_0) \leq f_i(x_0) \leq f_i^U(x_0), 
$$
$$
f_i^L(x_0)=f_i(x_0+\epsilon_{a_i}), f_i^U(x_0)=f_i(x_0+\epsilon_{b_i}), i \in \{1,\cdots,k\}
$$
...
$$
$$
f_i^L(x_0)=f_i(x_0+\epsilon_{a_i\prime}), f_i^U(x_0)=f_i(x_0+\epsilon_{b_i\prime}), i \in \{1,\cdots,k\}
$$
且
...
我们称模型在$x_0$点处具有可验证鲁棒性，即在扰动区域内不存在扰动可以使得模型输出错误的标签，即对于$\forall x \in \mathcal{B}_p(x_0,\epsilon),\argmax_{i\in \{1,\cdots,k\}}f_i(x)=y_{true}$。
...
$$
Verrfied Error:=\frac{\mathbb{I}(\argmax_{i\in \{1,\cdots,k\}}f_i(x_0)=c)\quad \&\quad \mathbb{I}(\mathcal{S}(x_0,\epsilon)_{i \neq c} \ge 0))}{n}
$$
